# Spec Validation Assertions

*Normative original: [${CLAUDE_PLUGIN_ROOT}/adoption/checks/spec-validation.md](${CLAUDE_PLUGIN_ROOT}/adoption/checks/spec-validation.md). This file copies its assertions in full so the skill is usable without hunting; if the two ever disagree, the original governs.*

Runs on `practice-spec.md` before the gate. **Any failure sends the run backward, never forward** — back to Step 3 (elicit) or Step 5 (write the spec) of [the procedure](../SKILL.md), never to the gate with a caveat.

Every assertion here is mechanical: a yes/no answer that costs no opinion about whether the practice is good. That is the point — the gate spends the approver's judgment on whether this is the right practice, not on whether the spec is internally sound.
...
T2 matters most. It is the direct analogue of a coverage check: an element with no disposition is a requirement that vanished between reading and spec, and nothing else in the loop will notice.
...
C1 is worth running literally, as a text scan, not as a judgment. It is the single highest-value assertion in this file — the failure it catches is the most common way a best practice dies and the easiest one to miss by reading.
...
Three of these have no analogue in a design that surveys fresh each run, and they are worth running first: **C1** (no unfilled placeholder), **H5** (no untested enforcement), and **N5** (no contended path). Each catches a failure that is invisible on reading and expensive to discover later.
